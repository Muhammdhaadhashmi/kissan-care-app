...
public class FarmerManager {
    ...
    public void setUserDetails(String key, FarmerUserModel val) {
        mEditor.putString(user, new Gson().toJson(val));
        mEditor.commit();
    }

    public FarmerUserModel getUserDetails(String key) {
        return new Gson().fromJson(mPrefs.getString(user, ""), FarmerUserModel.class);
    }
